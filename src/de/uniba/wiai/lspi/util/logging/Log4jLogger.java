 ...
 * 
 * <br/>
 * To configure this logger you have to set the system property
...
public class Log4jLogger extends Logger {
	...
	/**
	 * Name of property to set the reference to the property file containing log4j
	 * properties.
	 */
	...
	private static void configure() {
		if (!configured) {
			...
			try {
				configURL = ClassLoader.getSystemClassLoader()
						.getResource(System.getProperty(PROPERTIES_FILE_PROPERTY));
				if (configURL == null) {
			...
			if (usefile) {
				System.out.println("[" + Thread.currentThread().getName() + "] " + "INFO " + Log4jLogger.class.getName()
						+ " - Configuring log4j with '" + System.getProperty(PROPERTIES_FILE_PROPERTY) + "'.");
				try {
					...
					System.out.println("[" + Thread.currentThread().getName() + "] " + "ERROR "
							+ Log4jLogger.class.getName() + " - log4j could not be configured with '"
							+ System.getProperty(PROPERTIES_FILE_PROPERTY) + "'.");
				}
			} else {
