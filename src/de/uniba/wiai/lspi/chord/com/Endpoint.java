 ...
 * state is made by invocation of {@link #listen()}.
 * 
 * 
 * <br/>
 * <br/>
...
public abstract class Endpoint {
	 ...
	 * {@link #ACCEPT_ENTRIES}. Remember to eventually edit this array if you change
	 * the methods in interface {@link Node}. The method names contained in this
	...
	/**
	 * {@link EndpointStateListener listeners}interested in state changes of this
	 * endpoint.
	 */
	 ...
	 * 
	 * @param s The integer identifying the state to that the endpoint switched. See
	 *          {@link Endpoint#ACCEPT_ENTRIES}, {@link Endpoint#DISCONNECTED},
	...
	/**
	 * Tell this endpoint that it can listen to incoming messages from other chord
	 * nodes. TODO: This method may throw an exception when starting to listen for
	 ...
	 * make it possible for other chord nodes to connect to the node on that this
	 * endpoint invocates methods.
	 * 
	 * TODO: This method may throw an exception when starting to listen for incoming
	 * connections.
	 */
	protected abstract void openConnections();

	/**
	 * Tell this endpoint that the node is now able to receive messages that request
	 * the storage and removal of entries.
	 * 
	 */
	...
	/**
	 * Tell this endpoint to disconnect and close all connections. If this method
	 * has been invoked the endpoint must be not reused!!!
	 */
	...
	/**
	 * Create the endpoints for the protocol given by <code>url</code>. An URL must
	 * have a known protocol. An endpoint for an {@link URL} can only be create once
	 * and then be obtained with help of {@link Endpoint#getEndpoint(URL)}. An
	 * endpoint for an url must again be created if the
	 * {@link Endpoint#disconnect()} has been invoked.
	 * 
	 * @param node The node to which this endpoint delegates incoming requests.
	 * @param url  The URL under which <code>node</code> will be reachable by other
	 *             nodes.
	 * @return The endpoint created for <code>node</code> for the protocol specified
	 *         in <code>url</code>.
	 * @throws RuntimeException This can occur if any error that cannot be handled
	 *                          by this method occurs during endpoint creation.
	 */
		...
		synchronized (endpoints) {
			...
			if (url.getProtocol().equals(URL.KNOWN_PROTOCOLS.get(URL.SOCKET_PROTOCOL))) {
				...
				throw new IllegalArgumentException(
						"Url does not contain a " + "supported protocol " + "(" + url.getProtocol() + ")!");
			}
